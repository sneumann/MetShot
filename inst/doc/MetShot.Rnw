...
ms2peaks <- findPeaks(msn2xcms(xraw), method="centWave",
...
You can plot the raw MS2 data, and superimpose the precursor

<<plotMS2raw, fig = TRUE, eps = FALSE>>=

plotGroups(xraw, 
           precursor=ms2precursor,
           xa=xa,
           ms1peaks=ms1peaks,
           precursorcol=1)
...
## segments(ms1pre[,"rt"], ms1pre[,"mz"],
##          psppre[,"rt"], psppre[,"mz"],
##          col="blue")

## library(class)
## ms1pre <- peaks(ms1peaks)[knn(ms1, psppre, 1:nrow(ms1)),c("rt","mz", "into")]
## points(ms1pre, col="darkred", pch=".", cex=3)
